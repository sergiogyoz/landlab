...
cdef _id_array_contains(DTYPE_t *array, DTYPE_t size, DTYPE_t id):
    cdef long n
    for n in range(size):
        if array[n] == id:
            return 1
...
def id_array_contains(
    np.ndarray[DTYPE_t, ndim=2, mode="c"] corners_at_cell not None,
    np.ndarray[DTYPE_t, ndim=1, mode="c"] n_corners_at_cell not None,
    ...
    for cell in range(n_cells):
        out[cell] = _id_array_contains(&corners_at_cell[cell, 0], n_corners_at_cell[cell], id)
